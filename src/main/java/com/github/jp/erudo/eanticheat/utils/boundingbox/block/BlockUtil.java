package com.github.jp.erudo.eanticheat.utils.boundingbox.block;
...
public class BlockUtil {
  ...
  /*  public static boolean isOnFuckingBoat(Player player) {
    ...
    public static List<BoundingBox> getBlockBoundingBox(Block block) {
        List<BoundingBox> boxes = EAC.getInstance().getBlockBoxManager().getBlockBox().getSpecificBox(block.getLocation());
    ...
    public static List<Block> getBlocks(BoundingBox box, World world) {
        ...
        EAC.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(world, box).forEach(box2 -> BlockUtil.getBlock(box2.getMinimum().toLocation(world)));
        return block;
    }
