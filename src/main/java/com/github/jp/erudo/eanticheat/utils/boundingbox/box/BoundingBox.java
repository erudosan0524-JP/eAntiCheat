...
public class BoundingBox {
        ...
        for (int x = minX; x < maxX; x++) {
            for (int z = minZ; z < maxZ; z++) {
                for (int y = minY - 1; y < maxY; y++) {
                    ...
                    if (EAC.getInstance().getBlockBoxManager().getBlockBox().isChunkLoaded(loc)) {
                        Block block = BlockUtil.getBlock(loc);
                        if (!block.getType().equals(Material.AIR)) {
                            toReturn.addAll(EAC.getInstance().getBoxes().getBoundingBox(block));
                        }
                    }
                }
            }
    ...
    public Object toAxisAlignedBB() {
        return ReflectionUtils.newAxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ);
    }
