...
import com.github.jp.erudo.eanticheat.utils.TimeUtils;
...
public class MovementProcessor {
    ...
    public void update(Object packet, String type) {
        if (user != null) {
            ...
            if (type.equalsIgnoreCase(Packet.Client.ENTITY_ACTION)) {
                WrappedInEntityActionPacket
                wrappedInEntityActionPacket = new WrappedInEntityActionPacket(packet, user.getPlayer());
    ...
    private void updateBlockCheck() {
        ...
        List<BoundingBox> boxes = EAC.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(user.getPlayer().getWorld(), user.getBoundingBox().grow(0.5f, 0.35f, 0.5f).subtract(0, 0.5f, 0, 0, 0, 0));
